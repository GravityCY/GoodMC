...
public class BiomeLocatable implements IMovementLocatable {
    ...
    public static void updateLocator(ItemStack compass, ServerWorld serverWorld, ServerPlayerEntity serverPlayer) {
        ...
        if (!BetterCompass.isPointingAtPosition(compass)) {
            ...
            double distPrev = Math.sqrt(playerPos.getSquaredDistance(BetterCompass.getPointPosition(compass)));
...
//                BetterCompass.setPointStrength(compass, Math.max(Math.min(500d / distPrev, 1), 0.25d));
                return;
            }
        }

        BetterCompass.setPointPosition(compass, locatedPos);
//        BetterCompass.setPointStrength(compass, Math.max(Math.min(500d / distLocated, 1), 0.25d));
